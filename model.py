...
class Attention(chainer.Chain):
    ...
    def __init__(self, n_mid_units, n_out, win_size, batch_size, att_units_size, device=-1):
        ...
        self.Zu = xp.zeros((batch_size,n_out+16),dtype=np.float32)
        self.pad_zero=xp.zeros((self.pad_size,n_mid_units),dtype=self.xp.float32)

        initializer = chainer.initializers.LeCunNormal()

        with self.init_scope():
            self.last_out = L.Linear(n_out+16, self.att_units_size,initialW=initializer)
    ...
    def __call__(self, x):
        ...
        for i in range(len(gts)-self.pad_size*2):
            ...
            attend=F.softmax(
                F.tanh(
                    self.att_cal(attend.reshape(-1,self.att_units_size))
            ...
            result_type.append(self.type_output(context))
            self.Zu=F.concat([result[-1],result_type[-1]]).data
